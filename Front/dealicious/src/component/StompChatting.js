...
const StompChatting = () => {
  ...
  useEffect(() => { //컴포넌트가 마운트될 때 connect() 함수를 호출하여 Stomp 클라이언트를 연결하고, 컴포넌트가 언마운트될때  disconnect() 함수를 호출하여 연결을 끊습니다.
    axios.get(`http://43.203.108.152:8090/chatroom/` + channelId, {
      headers: {
        Authorization: token,
      }
    })
  ...
  const receipt = () => {
    console.log("수령완료");
    axios.get(`http://43.203.108.152:8090/receipt/` + sale.num, {
      headers: {
        Authorization: token,
      }
    })
  ...
  const handleRegister = (e) => {
    ...
    axios.get(`http://43.203.108.152:8090/review/${fixedRating}/${chatpartner.email}/${sale.num}`, {
      headers: {
        Authorization: token,
      }
    })
      ...
      <div style={{ marginTop: "20px", width: "385px", borderTop: "1px solid gray", borderBottom: "1px solid gray", height: "105px" }}>
        <div style={{ marginTop: "10px", marginBottom: "10px" }}>
          <div style={{ display: "flex" }}>
            <div><img src={`43.203.108.152/img/${fileurlList[0]}`} style={{ width: "80px" }}></img></div>
            ...
            item.type == "chat" ?
              item.writerId == user.email ?
                ...
                <div style={{ textAlign: "left", marginBottom: "15px" }}>
                  <div style={{ display: "inline-block", marginRight: "8px" }}>{chatpartner.profileimgurl==null ? <img src='/profile.png' style={{ width: "50px" }}></img>:<img src={`43.203.108.152/img/${chatpartner.profileimgurl}`} style={{ width: "50px" }}></img>}</div>
                ...
                <div style={{ borderLeft: "3px solid #D9D9D9", paddingLeft: "10px", textAlign: "left", marginBottom: "15px" }}>
                  ...
                  <Modal className='main' style={{
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      <div><img src={`43.203.108.152/img/${fileurlList[0]}`} style={{width:"100px"}}/></div>
