...
const Mypage = () => {
    ...
    useEffect(() => {
        axios.get(`http://43.203.108.152:8090/mypagelist/${user.email}`)
            .then(res => {
                                    ...
                                    <div style={{ display: "inline-block", paddingRight: index % 3 === 2 ? "0px" : "10px" }}>
                                        {item.status === "거래완료" ? (
                                            <div style={{ width: "120px", height: "120px", borderRadius: "10px", position: "relative", opacity: "0.5" }}>
                                                <img src={`http://43.203.108.152:8090/img/${item.fileurl.split(',')[0]}`} style={{ width: "120px", height: "120px", borderRadius: "10px" }} />
