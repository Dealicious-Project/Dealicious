...
function ChatList() {
    ...
    useEffect(() => { //컴포넌트가 마운트될 때 connect() 함수를 호출하여 Stomp 클라이언트를 연결하고, 컴포넌트가 언마운트될때  disconnect() 함수를 호출하여 연결을 끊습니다.
        axios.get(`http://43.203.108.152:8090/chatroomlist`, {
            headers: {
                Authorization: token,
            }
        })
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        <td rowSpan={2}><img src={`http://43.203.108.152:8090/img/${item.fileurl.split(',')[0]}`} alt='' style={{ width: "50px", height: "50px" }} /></td>
                                    </tr>
                                    <tr>
